...
#define MODAL_GROUP_G0 \
  0  // [G4,G10,G28,G28.1,G30,G30.1,G53,G92,G92.1] Non-modal
...
#define MODAL_GROUP_G7 \
  7  // [G40] Cutter radius compensation mode. G41/42 NOT SUPPORTED.
#define MODAL_GROUP_G8 8  // [G43.1,G49] Tool length offset
#define MODAL_GROUP_G12 \
  9  // [G54,G55,G56,G57,G58,G59] Coordinate system selection
...
#define MOTION_MODE_PROBE_TOWARD \
  4  // G38.2 NOTE: G38.2, G38.3, G38.4, G38.5 must be sequential. See
     // report_gcode_modes().
...
typedef struct {
  ...
  float coord_system[N_AXIS];  // Current work coordinate system (G54+). Stores
                               // offset from absolute machine position in mm.
                               // Loaded from EEPROM when called.
...
typedef struct {
  //   uint16_t command_words;  // NOTE: If this bitflag variable fills, G and M
  //   words can be separated. uint16_t value_words;
