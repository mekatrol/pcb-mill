...
static bool eeprom_ack_polling(void) {
  ...
  while (get_systick() < expires) {
    ...
    // Just sit idle for a bit so we are not hammering get_systick
    for (int i = 0; i < 100; i++);
  }
...
uint8_t eeprom_get_char_hal(uint32_t addr) {
  return at24c32_read_byte(addr);
}

void eeprom_put_char_hal(uint32_t addr, uint8_t new_value) {
  at24c32_write_byte(addr, new_value);
}
