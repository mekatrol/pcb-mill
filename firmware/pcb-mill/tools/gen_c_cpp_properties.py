...
board_name_pattern = re.compile(r"^\s*BOARD_NAME\s*(?P<assign_op>:=|\+?=)\s*(?P<board_name>.+)$")
board_name = ""
...
with open(makefile_path) as f:
    for lineno, line in enumerate(f, start=1):
        ...
        # Is this the board name?
        m = board_name_pattern.match(line)
        if m:
...
if board_name == "":
    print("⚠️  Warning: board name not defined")
