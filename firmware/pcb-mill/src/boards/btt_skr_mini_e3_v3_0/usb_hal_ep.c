...
void dcd_edpt0_status_complete(const usb_control_request_t *request) {
...
static bool endpoint_transfer_hal(uint8_t ep_num, usb_endpoint_direction_index_t dir) {
  endpoint_packet_t *control_transfer = endpoint_buffer_state(ep_num, dir);
...
bool usb_endpoint_transfer_hal(uint8_t ep_num, uint8_t ep_dir_idx, uint8_t *buffer, uint16_t total_bytes) {
  ...
  return endpoint_transfer_hal(ep_num, ep_dir_idx);
}
